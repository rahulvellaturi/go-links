 ...
 * API. The DB file is created at the project root on first run.
 *
 * Statements are prepared per call rather than cached for the app's lifetime:
 * node:sqlite can finalize long-lived prepared statements, so preparing at the
 * point of use is the reliable pattern at this scale.
 */
...
export function getLink(shortname: string): Link | undefined {
  const row = db
    .prepare('SELECT * FROM links WHERE shortname = ?')
    .get(normalise(shortname)) as unknown as Row | undefined;
...
export function recordVisit(shortname: string): void {
  db.prepare(
    `UPDATE links
     ...
     WHERE shortname = ?`,
  ).run(new Date().toISOString(), normalise(shortname));
}

/** Insert a few example links the first time the app runs, so the list isn't empty. */
function seedIfEmpty(): void {
  const n = (db.prepare('SELECT COUNT(*) AS n FROM links').get() as unknown as { n: number }).n;
  if (n > 0) return;
